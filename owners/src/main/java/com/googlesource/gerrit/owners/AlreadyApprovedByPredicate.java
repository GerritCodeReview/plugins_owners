      ...
      if (!filesOwnedByApprover.isEmpty()) {
        ...
        boolean allChangesToOwnedFilesAreDueToRebase =
            filesOwnedByApprover.stream()
                .allMatch(
                    ownedFile ->
                        /* In case of a rename, priorVsCurrent key will be the **new** path, not the **old** one,
                          so if the approver owns only the old path of a renamed file, we treat it as requiring
                          re-approval
                        */
