...
## uploaderin:already_approved_by_owners
...
When evaluating a label's `copyCondition`, the operand `uploaderin:already_approved_by_owners`
...
Given an `OWERS` file that split ownerships between `frontend` users and `backend` users based on
file matchers:
...
* The `user-fronted` owner does not own any of the modified files.
...
**Result**: The approval is cleared because the owner must verify the new changes.

## Label configuration

The copy condition is a `label` property and as such needs to be set un in the label configuration.
The following is an example of the `approverin:already_approved_by_owners` configured for the
`Code-Review` label.
...
[label "Code-Review"]
    ...
    copyCondition = approverin:already_approved_by_owners
```
