...
When the `auto-owners-approved` is set to `false`, at global level in the `OWNERS` file of the
project or path, or for one of the files matching a section, then the
copy condition is not satisfied and therefore the scores are not copied over to the new patch-set.
