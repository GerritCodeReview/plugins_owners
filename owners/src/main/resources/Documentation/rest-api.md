...
`files_auto_approved` is a subset of `files_approved`. It contains the files whose approval on the
current patch set comes from a vote that was copied forward because the `auto-owners-approved`
logic applies.

If a file also has a sufficient explicit owner vote on the current patch set, it is still returned
in `files_approved`, but it is not returned in `files_auto_approved`.
