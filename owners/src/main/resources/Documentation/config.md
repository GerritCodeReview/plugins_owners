...
repository unless configured otherwise. In real repositories this is often too
coarse. Some paths are sensitive and should always require fresh owner review,
while other paths benefit from keeping owner approvals across patch sets.
...
If the field is not set, the effective value is `true`.

### Inheritance

The usual `OWNERS` inheritance logic applies to `auto-owners-approved` as well.
This includes directory `OWNERS` lookup, project `refs/meta/config` `OWNERS`, and
parent project `OWNERS` when inheritance continues up the project hierarchy.

1. At `OWNERS` file level, the field follows normal `OWNERS` inheritance. If
   inherited is `true` and the field is missing, the value is inherited from
   parent OWNERS entries, including `refs/meta/config` and parent projects when
   applicable.
2. If inherited is `false`, inheritance stops at that level and a missing value
   falls back to `true`.
3. Matcher values are local overrides and take precedence over the `OWNERS` file
   level value for files matched by that matcher.
4. If multiple regular matchers match the same file and define this field, the
   effective value is true only when all those matcher values are true. Any
...
Disable by default, enable for documentation files:
...
have a governance team explicitly approving changes but do not wish to necessarily grant
...
> **NOTE:** You can also use `label: Code-Review, 1` as your definition of submittability
