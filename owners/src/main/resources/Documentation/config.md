...
:   If set to `true` the approvals are evaluated through the owners plugin
    ...
    > **Note:** the `owners.enableSubmitRequirement = true` is a global
    > setting and allows for quick site switch from `prolog` submit rule to
    > plugin's provided submit rule. However in case when such migration is not
    > feasible (e.g. when `prolog` rules need to be slowly phased out or when
    > more control is needed over rule's applicability) one can turn submit
    > requirement in `project.config` for a certain project (or hierarchy of
    > projects), without turning it on globally, as `approval_owners` predicate
    > is _always_ available. Please also note, that project's `rules.pl` should
    > be removed in this case so that it doesn't interfere with change
    > evaluation. The minial configuration looks like below (see
