...
public abstract class GetFilesOwnersITAbstract extends LightweightPluginDaemonTest {
  ...
  public void shouldReturnFilesAutoApprovedWhenOwnerVoteIsCopied() throws Exception {
    ...
    assertFilesApproval(changeId.toString(), OWNED_JAVA_FILE, owners(admin), owners(admin));
  }
  ...
  private void assertFilesApproval(
      String changeId,
      String filePath,
      java.util.Set<GroupOwner> approvedOwners,
      java.util.Set<GroupOwner> autoApprovedOwners)
      throws Exception {
    Response<FilesOwnersResponse> response = responseFor(changeId);
    assertThat(response.value().files).isEmpty();
    assertThat(response.value().filesApproved).containsExactly(filePath, approvedOwners);
  ...
  private Response<FilesOwnersResponse> responseFor(String changeId) throws Exception {
    return assertResponseOk(ownersApi.apply(parseCurrentRevisionResource(changeId)));
  }
