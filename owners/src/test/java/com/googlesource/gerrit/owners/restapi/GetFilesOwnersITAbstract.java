...
public abstract class GetFilesOwnersITAbstract extends LightweightPluginDaemonTest {
  ...
  private static final String UNOWNED_TXT_FILE = "a.txt";
  ...
  public void shouldReturnFilesAutoApprovedWhenOwnerVoteIsCopied() throws Exception {
    ...
    ChangeIdentifier changeId = createChangeWithCopiedOwnerVote(admin);
  ...
  public void shouldReturnFilesAutoApprovedWhenNextPatchSetAddsOnlyOwnedFile() throws Exception {
    setupAutoApprovalForJavaMatcher(admin);
    String ANOTHER_OWNED_JAVA_FILE = "another-" + OWNED_JAVA_FILE;
    ...
    assertThat(response.value().filesAutoApproved())
        .containsExactly(OWNED_JAVA_FILE, owners(admin), ANOTHER_OWNED_JAVA_FILE, owners(admin));
  }
  ...
  private ChangeIdentifier createChangeWithCopiedOwnerVote(TestAccount owner) throws Exception {
    ChangeIdentifier changeId =
        changeOperations
            .newChange()
            .project(project)
  ...
  private void setupAutoApproval(String ownersContent) throws Exception {
    updateLabel(b -> b.setCopyCondition("approverin:" + FULL_OPERAND_WITH_PLUGIN_NAME));
    merge(createChange(testRepo, "master", "Add OWNER file", "OWNERS", ownersContent, ""));
  }
