...
public class GetFilesOwnersIT extends LightweightPluginDaemonTest {
  ...
  public void shouldReturnACorrectResponse() throws Exception {
    ...
    HashMap<String, List<Owner>> responseValue = (HashMap<String, List<Owner>>) resp.value();
    HashMap<String, List<Owner>> expectedResponseValue = new HashMap<String, List<Owner>>() {
      {
        put(
                "a.txt",
    ...
    assertThat(responseValue.keySet()).isEqualTo(expectedResponseValue.keySet());
    assertThat(responseValue.values().size()).isEqualTo(expectedResponseValue.values().size());
    assertThat(responseValue.values().stream().findFirst().get().get(0).getId()).isEqualTo(expectedResponseValue.values().stream().findFirst().get().get(0).getId());
    assertThat(responseValue.values().stream().findFirst().get().get(0).getName()).isEqualTo(expectedResponseValue.values().stream().findFirst().get().get(0).getName());
  }
