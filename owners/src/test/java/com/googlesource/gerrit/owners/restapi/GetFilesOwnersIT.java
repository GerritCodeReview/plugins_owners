...
public class GetFilesOwnersIT extends LightweightPluginDaemonTest {
  ...
  public void shouldReturnACorrectResponse() throws Exception {
    ...
    final Collection<Set<Owner>> owners = responseValue.values();
    final Collection<Set<Owner>> expectedOwners = expectedResponseValue.values();
    assertThat(owners.size()).isEqualTo(expectedOwners.size());

    Owner owner = Iterables.getFirst(owners.stream().findFirst().get(), null);
    assertThat(owner.getId()).isEqualTo(admin.id().get());
    assertThat(owner.getName()).isEqualTo(admin.fullName());
  }
