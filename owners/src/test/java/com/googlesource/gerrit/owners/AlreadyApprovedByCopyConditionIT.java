...
public class AlreadyApprovedByCopyConditionIT extends LightweightPluginDaemonTest {
  ...
  @Test
  public void shouldCopyApprovalWhenAllEditsToOwnedFileAreDueToRebaseEvenIfAnUnrelatedFileWasAdded()
      throws Exception {
    ObjectId initialCommitId = createInitialContentFor(BACKEND_OWNED_FILE);
    PushOneCommit.Result amendL3 =
        createChangeWithReplacedContent(BACKEND_OWNED_FILE, "Line 3\n", "Line three\n");
  ...
  public void shouldCopyApprovalWhenRemovingOwnedFileAndRebaseOnChangeThatRemovesTheSameFile()
    ...
    Change.Id removedFileChangeId =
        changeOperations.newChange().file(BACKEND_OWNED_FILE).delete().create();
    ...
    RevisionInfo currentRevision =
        gApi.changes().id(project.get(), baseRemovedFileChangeId.get()).get().getCurrentRevision();
    ObjectId baseCommit = ObjectId.fromString(currentRevision.commit.commit);
    assertThat(baseCommit).isNotNull();
    rebaseChangeOn(removedFileChangeId.toString(), baseCommit);
