...
suite('owners status tests', () => {
  ...
  suite('getChangeAccounts tests', () => {
    test('getChangeAccounts - should return empty map when change is `undefined', () => {
      const undefinedChange = undefined;
      const [accountsById, accountsByEmailWithoutDomain, accountsByFullName] =
        getChangeAccounts(undefinedChange);
      assert.equal(accountsById.size, 0);
      assert.equal(accountsByEmailWithoutDomain.size, 0);
      assert.equal(accountsByFullName.size, 0);
    });
    ...
    test('getChangeAccounts - should return map with owner when change has only owner and empty reviewers defined', () => {
      ...
      const [accountsById, accountsByEmailWithoutDomain, accountsByFullName] =
        getChangeAccounts(changeWithOwner);
      assert.equal(deepEqual(accountsById, new Map([[1, owner]])), true);
      assert.equal(
        deepEqual(accountsByEmailWithoutDomain, new Map([['1_email', owner]])),
        true
      );
      assert.equal(
        deepEqual(accountsByFullName, new Map([['1_name', owner]])),
        true
      );
    });
