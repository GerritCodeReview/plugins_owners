 ...
 */
export class OwnersApiCache {
  private cached = new Map<string, Promise<unknown>>();
  ...
  private fetch(key: string, loader: () => Promise<unknown>): Promise<unknown> {
  ...
  getLoggedInUser(): Promise<User> {
    return this.fetch('getLoggedInUser', () =>
      this.getLoggedInUserImpl()
    ) as Promise<User>;
  }

  getFilesOwners(): Promise<FilesOwners | undefined> {
    return this.fetch('getFilesOwners', () =>
      this.getFilesOwnersImpl()
    ) as Promise<FilesOwners | undefined>;
  }
