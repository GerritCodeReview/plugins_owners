...
suite('owned files tests', () => {
  suite('ownedFiles tests', () => {
    ...
    test('ownedFiles - should not mark file as auto approved for owner outside auto approvers', () => {
      const anotherOwner = account(2);
      const filesOwnersWithDifferentAutoApprover = {
        files_approved: {
          [ownedApprovedFile]: [fileOwner(1), fileOwner(2)],
        },
        files_auto_approved: {
          [ownedApprovedFile]: [fileOwner(1)],
        },
      ...
      assert.equal(
        deepEqual(ownedFiles(anotherOwner, filesOwnersWithDifferentAutoApprover), {
          ownedFiles: [{file: ownedApprovedFile, status: FileStatus.APPROVED}],
          numberOfApproved: 1,
          numberOfPending: 0,
        }),
