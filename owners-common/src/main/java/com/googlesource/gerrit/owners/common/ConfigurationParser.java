...
public class ConfigurationParser {
  ...
  private Optional<Matcher> toMatcher(JsonNode node) {
    ...
    Optional<Matcher> suffixMatcher =
        getText(node, "suffix").map(el -> new SuffixMatcher(el, owners, reviewers, group_owners));
    Optional<Matcher> regexMatcher =
        getText(node, "regex").map(el -> new RegExMatcher(el, owners, reviewers, group_owners));
    Optional<Matcher> partialRegexMatcher =
        getText(node, "partial_regex")
            .map(el -> new PartialRegExMatcher(el, owners, reviewers, group_owners));
    Optional<Matcher> exactMatcher =
        getText(node, "exact").map(el -> new ExactMatcher(el, owners, reviewers, group_owners));
    Optional<Matcher> genericMatcher =
        getText(node, "generic").map(el -> new GenericMatcher(el, owners, reviewers, group_owners));
