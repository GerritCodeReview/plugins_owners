...
public class PathOwnersTest extends ClassicConfig {
  ...
  public void testClassic() throws Exception {
    ...
    PathOwners owners =
        new PathOwners(accounts, Optional.of(allProjectsRepository), repository, branch, patchList);
  ...
  public void testDisabledBranch() throws Exception {
    ...
    PathOwners owners =
        new PathOwners(
            accounts, Optional.of(allProjectsRepository), repository, Optional.empty(), patchList);
    Set<Account.Id> ownersSet = owners.get().get(CLASSIC_OWNERS);
