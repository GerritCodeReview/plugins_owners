...
public class GitRefListener implements GitReferenceUpdatedListener {
  ...
  public void processEvent(
    ...
    try {
      ...
      if (patchList != null) {
        PathOwners owners =
            new PathOwners(accounts, allProjectsRepository, repository, change.branch, patchList);
        for (String pattern : disabledBranchPatterns) {
          if (change.branch.matches(pattern)) {
            owners = new PathOwners(accounts, allProjectsRepository, repository, patchList);
            break;
          }
        }
